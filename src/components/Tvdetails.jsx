   ...
   const dispatch=useDispatch()
    ...
    return info ?(
        ...
        style={{
          ...
          <h1 className="text-xl my-3 font-bold text-white">tv Translation</h1>
          ...
          <div className=" flex flex-col gap-y-5 mt-5 w-[80%]">
           {info.watchproviders &&
            ...
            {info.watchproviders.flatrate.map((w,i)=>(
                <img key={i} title={w.provider_name} className="w-[5vh] h-[5vh] rounded-md" src={`https://image.tmdb.org/t/p/original/${w.logo_path}`}></img>
            ))}
            </div>
            )}
          ...
          <hr className="mt-5 border-none h-[3px] bg-zinc-500"></hr>
           {/* Seasons stuff 4*/}
           <h1 className="text-2xl font-bold text-white mt-3">Seasons</h1>
           <div className=" w-[100%]  flex overflow-y-hidden p-2 overflow-x-auto ">
            
            {info.detail.seasons.map((w,i)=>(
                <div className="w-[25vh] mr-5">
                
              <img className = "h-[30vh] min-w-[20vh] shadow-[8px_17px_38px_2px_rgba(0,0,0,.5)] rounded-md" src={w.poster_path || d+w.backdrop_path || w.profile_path?`https://image.tmdb.org/t/p/original/${w.poster_path || w.backdrop_path || w.profile_path}`:"PNA.jpg"}></img>
              <h1 className = " text-zinc-300 text-xl font-bold mt-3  ">{ w.name  }</h1>
          ...
          <hr className="mt-5 border-none h-[3px] bg-zinc-500"></hr>
           {/* recommdation and similar stuff 5*/}
           <div className="mt-5">
